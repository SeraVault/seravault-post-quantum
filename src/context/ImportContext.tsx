...
export const ImportProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const importItem = async (item: ImportItem, userId: string, userPrivateKey: string) => {
    ...
    if (item.type === 'account') {
      formData = await createFormFromTemplate('password', item.title, userId);
      formData.data = {
      ...
      formData = await createFormFromTemplate('secure_note', item.title, userId);
      formData.data = {
        title: item.title || '',
        content: item.notes || '',
      };
    } else if (item.type === 'credit_card') {
      formData = await createFormFromTemplate('credit_card', item.title, userId);
      formData.data = {
      ...
      formData = await createFormFromTemplate('crypto_wallet', item.title, userId);
      formData.data = {
