  ...
  async cacheFile(
    ...
    try {
      ...
      if (currentSize + blob.size > this.MAX_CACHE_SIZE) {
        console.warn('⚠️ Cache full, removing old files');
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
          console.log(`✅ Cached file ${fileId} for offline access (${(blob.size / 1024).toFixed(2)} KB)`);
          resolve();
        };
        request.onerror = () => reject(request.error);
      });
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
          ...
          console.log(`✅ Retrieved cached file ${fileId} from offline storage`);
          resolve(cached);
        };
        request.onerror = () => reject(request.error);
      });
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        transaction.oncomplete = () => {
          console.log(`🗑️ Removed cached file ${fileId}`);
          resolve();
        };
        ...
        fileRequest.onsuccess = async () => {
          ...
          for (let i = 0; i < filesToRemove; i++) {
            ...
            if (!metadata?.availableOffline) {
              ...
              console.log(`🗑️ Cleaned up old cached file: ${file.fileId}`);
            }
          }
        };
  ...
  async clearAll(): Promise<void> {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        transaction.oncomplete = () => {
          console.log('🗑️ Cleared all offline file cache');
          resolve();
        };
        transaction.onerror = () => reject(transaction.error);
      });
